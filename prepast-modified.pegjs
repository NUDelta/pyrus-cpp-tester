...
PrepFunctionMacroArgs = LPAR a:Identifier b:(COMMA c:Identifier {return c;})* RPAR {
    return [a].concat(b);
};

PrepFunctionMacroCallArgs = LPAR a:PrepMacroMacroCallText b:(COMMA c:PrepMacroMacroCallText {return c;})* RPAR {
    return [a].concat(b);
};
